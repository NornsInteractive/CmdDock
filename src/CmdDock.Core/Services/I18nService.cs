...
public partial class I18nService
{
    ...
    private static readonly Dictionary<string, string> _zhStrings = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        ["Preset.preset_disk_health.Desc"] = "查询本机所有物理硬盘的健康与运行状态（基于 Win 原生 PowerShell）",
        ...
        ["Preset.preset_docker_ps.Desc"] = "查看正在运行的 Docker 容器列表、状态和端口映射（需先安装 Docker）",
        ["Preset.preset_docker_prune.Name"] = "清理 Docker 虚悬缓存",
        ["Preset.preset_docker_prune.Desc"] = "一键清理无用的 Docker 悬空镜像、容器和网络构建缓存（需先安装 Docker）",
        ["Preset.preset_python_env.Name"] = "Python 依赖包列表",
        ["Preset.preset_python_env.Desc"] = "列出当前 Python 环境下所有已安装的第三方包版本（需先安装 Python）",
        ["Preset.preset_http_server.Name"] = "启动临时 HTTP 服务器",
        ["Preset.preset_http_server.Desc"] = "在 8080 端口启动 Windows 原生 HTTP 静态服务器（免装 Python，自动打开浏览器）",
        ["Preset.preset_wsl_terminal.Name"] = "进入 WSL Linux 环境",
        ["Preset.preset_wsl_terminal.Desc"] = "一键打开并进入 Windows Subsystem for Linux 终端环境（需先安装 WSL）",
        ["Preset.preset_git_config.Name"] = "Git 全局配置查看",
        ["Preset.preset_git_config.Desc"] = "在终端中展示 Git 的全局配置项与来源文件（需先安装 Git）",
        ...
        ["Preset.preset_git_status.Desc"] = "快速显示当前工作目录的 Git 分支、暂存区与未提交变更（需先安装 Git）",
        ["Preset.preset_node_version.Name"] = "Node & NPM 环境版本",
        ["Preset.preset_node_version.Desc"] = "速查当前系统配置的 Node.js 与包管理器运行版本（需先安装 Node.js）",
        ...
        ["Preset.preset_stop_wsl.Desc"] = "完全终止 WSL 实例，立即释放其占用的宿主机内存（需先安装 WSL）",
...
所有数据均存储在您的本地计算机中。您可以通过 Windows“设置”或微软商店直接卸载本应用，系统将彻底清除本应用及其关联的所有本地数据。",
        ...
        ["MiniDock.Title"] = "桌面小窗口",
        ["MiniDock.SwitchToMini"] = "切换为桌面小窗口",
        ["MiniDock.SwitchToMiniShort"] = "桌面小窗口",
        ["MiniDock.ExpandToFull"] = "展开为完整管理视图",
        ["MiniDock.CloseApp"] = "退出应用",
        ["MiniDock.ModeCard"] = "卡片控制面板模式",
        ["MiniDock.ModeBar"] = "紧凑 Dock 条模式",
        ["MiniDock.SwitchToCardDeck"] = "切换为卡片面板模式",
        ["MiniDock.SwitchToDockBar"] = "切换为紧凑 Dock 条模式",
        ["MiniDock.DockBarHandleToolTip"] = "按住拖动 / 双击展开大窗口",
        ["MiniDock.PinTop"] = "窗口始终置顶",
        ["MiniDock.PinDesktop"] = "固定在桌面底层 (壁纸层)",
        ["MiniDock.PinNormal"] = "取消固定 / 恢复普通窗口",
        ["MiniDock.Settings"] = "小窗口样式与设置",
        ["MiniDock.SettingsTitle"] = "小窗口设置",
        ["MiniDock.Opacity"] = "窗口不透明度",
        ["MiniDock.Scale"] = "卡片缩放",
        ["MiniDock.ScaleSmall"] = "紧凑小",
        ["MiniDock.ScaleMedium"] = "标准中",
        ["MiniDock.ScaleLarge"] = "宽阔大",
        ["MiniDock.AutoHide"] = "贴边自动隐藏",
        ["MiniDock.SearchPlaceholder"] = "搜索快捷指令...",
        ...
        ["MiniDock.AllCategories"] = "全部分类列表",
        ["MiniDock.ExecutionSuccess"] = "✓ 成功 (用时 {0}ms, 代码 {1})",
        ["MiniDock.ExecutionFailed"] = "✗ 失败 (代码 {0})",
        ["MiniDock.ExecutionOutput"] = "输出: {0}",
        ["Category.全部"] = "全部",

        // Settings - Mini Dock & Startup View
        ["Settings.MiniDockGroup"] = "桌面小窗口与启动视图",
        ["Settings.StartupView"] = "启动默认视图",
        ["Settings.StartupViewDesc"] = "选择打开 CmdDock 时默认呈现的窗口模式。",
        ["Settings.StartupViewFull"] = "完整管理大窗口 (默认)",
        ["Settings.StartupViewMini"] = "桌面小窗口 / Dock 栏",
        ["Settings.DockMode"] = "小窗口形态",
        ["Settings.DockModeDesc"] = "进入桌面小窗口时默认使用的布局外观。",
        ["Tray.OpenMain"] = "打开主窗口",
        ["Tray.OpenMini"] = "桌面小窗口",
        ["Tray.Exit"] = "退出应用"
    };
    ...
    private static readonly Dictionary<string, string> _enStrings = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        ["Preset.preset_disk_health.Desc"] = "Inspect local physical drive health status and operational state (Native PowerShell)",
        ...
        ["Preset.preset_docker_ps.Desc"] = "List all currently running Docker containers (Requires Docker)",
        ["Preset.preset_docker_prune.Name"] = "Prune Docker Cache",
        ["Preset.preset_docker_prune.Desc"] = "Clean unused dangling Docker images, containers, and build cache (Requires Docker)",
        ["Preset.preset_python_env.Name"] = "Python Pip Packages",
        ["Preset.preset_python_env.Desc"] = "List all installed Python package versions in current environment (Requires Python)",
        ["Preset.preset_http_server.Name"] = "Start Local HTTP Server",
        ["Preset.preset_http_server.Desc"] = "Launch Windows native HTTP static server on port 8080 (No Python required)",
        ["Preset.preset_wsl_terminal.Name"] = "Launch WSL Terminal",
        ["Preset.preset_wsl_terminal.Desc"] = "Open Windows Subsystem for Linux (WSL) interactive shell (Requires WSL)",
        ["Preset.preset_git_config.Name"] = "View Git Configuration",
        ["Preset.preset_git_config.Desc"] = "Display Git global configuration and origin files in terminal (Requires Git)",
        ...
        ["Preset.preset_git_status.Desc"] = "Inspect current Git branch, staging area, and uncommitted changes (Requires Git)",
        ["Preset.preset_node_version.Name"] = "Node & NPM Versions",
        ["Preset.preset_node_version.Desc"] = "Quickly check installed Node.js and npm runtime versions (Requires Node.js)",
        ...
        ["Preset.preset_stop_wsl.Desc"] = "Terminate WSL instances to instantly reclaim host RAM (Requires WSL)",
...
Uninstalling CmdDock via Windows Settings or Microsoft Store completely removes the application and all associated local data files.",
        ...
        // Mini Dock & Desktop Pinning
        ["MiniDock.Title"] = "Desktop Mini Dock",
        ["MiniDock.SwitchToMini"] = "Switch to Desktop Mini Dock",
        ["MiniDock.SwitchToMiniShort"] = "Mini Dock",
        ["MiniDock.ExpandToFull"] = "Expand to Full Management View",
        ["MiniDock.CloseApp"] = "Exit CmdDock",
        ["MiniDock.ModeCard"] = "Card Deck Panel Mode",
        ["MiniDock.ModeBar"] = "Compact Dock Bar Mode",
        ["MiniDock.SwitchToCardDeck"] = "Switch to Card Deck Panel Mode",
        ["MiniDock.SwitchToDockBar"] = "Switch to Compact Dock Bar Mode",
        ["MiniDock.DockBarHandleToolTip"] = "Drag to Move / Double-click to Expand",
        ["MiniDock.PinTop"] = "Always on Top",
        ["MiniDock.PinDesktop"] = "Pin to Desktop Layer",
        ["MiniDock.PinNormal"] = "Unpin / Normal Window",
        ["MiniDock.Settings"] = "Mini Dock Styles & Settings",
        ["MiniDock.SettingsTitle"] = "Mini Dock Settings",
        ["MiniDock.Opacity"] = "Window Opacity",
        ["MiniDock.Scale"] = "Card Scale",
        ...
        ["MiniDock.SearchPlaceholder"] = "Search shortcut commands...",
        ...
        ["MiniDock.AllCategories"] = "All Categories",
        ["MiniDock.ExecutionSuccess"] = "✓ Success ({0}ms, code {1})",
        ["MiniDock.ExecutionFailed"] = "✗ Failed (code {0})",
        ["MiniDock.ExecutionOutput"] = "Output: {0}",

        // Settings - Mini Dock & Startup View
        ["Settings.MiniDockGroup"] = "Desktop Mini Dock & Startup View",
        ["Settings.StartupView"] = "Default View on Startup",
        ["Settings.StartupViewDesc"] = "Choose the default window mode when opening CmdDock.",
        ["Settings.StartupViewFull"] = "Full Management Window (Default)",
        ["Settings.StartupViewMini"] = "Desktop Mini Dock / Dock Bar",
        ["Settings.DockMode"] = "Mini Dock Form Factor",
        ["Settings.DockModeDesc"] = "Default layout appearance when entering Mini Dock.",
